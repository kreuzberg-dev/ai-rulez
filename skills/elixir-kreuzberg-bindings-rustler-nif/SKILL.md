...
1. Deserialize using serde_json::from_value() for nested structures (ocr, chunking, images, pages, etc.)
1. Explicitly handle top-level booleans (use_cache, enable_quality_processing, force_ocr) for compatibility
1. Return default ExtractionConfig if parsing fails at any step
