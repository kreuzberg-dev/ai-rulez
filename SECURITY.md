...
This repository holds shared AI-governance configuration that other repositories render into agent instruction files. The security-relevant surface is **instruction injection**: content here is consumed by coding agents with write access to other repositories, so a rule that directs an agent to exfiltrate data, weaken a check, or run an arbitrary command is a genuine vulnerability. Report it.
