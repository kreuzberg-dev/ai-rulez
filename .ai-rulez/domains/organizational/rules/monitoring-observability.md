...
Standards for structured logging, metrics, and health checks across kreuzberg.dev services.

## Structured Logging

- Use `tracing` crate (Rust) or `structlog` (Python). Key=value pairs, not f-strings.
...
- Expose `/health` endpoint returning JSON status with component checks
- Wire to Kubernetes liveness/readiness probes
