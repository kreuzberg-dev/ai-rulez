...
3. **Feature gates**: Compile only needed functionality
4. **Minimal dependencies**: Avoid large transitive deps
...
2. Mutable global state shared with JavaScript
3. Panicking instead of returning Err()
4. Large monolithic WASM binary (split into modules)
5. Synchronous I/O operations
6. Assuming SharedArrayBuffer availability
7. Using threads without Web Workers coordination
8. Ignoring bundle size; always measure

**Good alternatives:**

1. Delegate I/O to JavaScript; WASM computes only
2. Pass data in/out, no shared mutable state
3. Return Result<T, JsValue> for all fallible ops
4. Use dynamic imports; split code by feature
5. All I/O async via Promises
6. Document SharedArrayBuffer assumptions; test availability
7. Use Web/Worker Threads for parallelism; coordinate via message passing
8. Measure size continuously; optimize bundling
