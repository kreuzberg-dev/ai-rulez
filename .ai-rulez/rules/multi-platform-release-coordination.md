...
2. **Dependency simplification**: Intercomponent versioning straightforward
3. **Security parity**: Security fixes apply uniformly across releases
4. **Change tracking**: Changelog entries consolidated by release date
5. **Documentation**: Single release announcement covers all ecosystems
...
2. **Update version in Cargo.toml only**
   ```toml
   [workspace.package]
   version = "0.8.3"
   ```

3. **Synchronize versions across ecosystems**
   ```bash
   task sync-versions
   ```

4. **Update CHANGELOG.md** for all ecosystems
...
6. **Push release branch and open PR**
   ```bash
   git push origin release/v0.8.3
   gh pr create --title "Release v0.8.3" --body "..."
   ```

7. **Wait for CI approval** (1-2 hours)
   - All tests pass
   - All security checks pass
   - Version consistency verified
   - Coverage thresholds met

8. **Merge release PR** (T-2 hours before publishing)
...
2. **Attach Release Artifacts** (if applicable)
   - Compiled binaries (CLI, extensions)
   - Checksums file (SHA256)
   - GPG signature files
   - Documentation bundle

3. **Publishing GitHub Release**
...
3. **Update release commit** with fixes
4. **Re-run tests** and security checks
5. **Merge when ready**
...
2. **Document failure reason** in issue
3. **Prepare patch release** with fix
4. **Yank problematic version** from all published registries
...
2. **Patch release prepared** within 24 hours
3. **Versions yanked** from registries
4. **Users notified** to upgrade to patched version
5. **Post-mortem** scheduled to prevent recurrence
