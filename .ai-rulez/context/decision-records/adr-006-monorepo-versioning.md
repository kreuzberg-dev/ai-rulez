...
2. **Synchronized versioning:** All packages use the same version number (simplicity, consistency)
...
2. Run release automation that builds and publishes all bindings
3. All bindings are released with the same version number simultaneously
4. A single release notes document covers all changes across all bindings
...
2. **Update version:** Increment version in:
...
3. **Run test suite:** Ensure all fixtures pass on all bindings
4. **Generate release notes:** Create consolidated changelog
5. **Tag release:** Create single git tag `v0.X.Y`
6. **Publish:** Automated CI/CD publishes to all package managers simultaneously
7. **Verify:** Confirm all packages appear in their respective package managers
...
5. Document what triggered the exception in release notes
