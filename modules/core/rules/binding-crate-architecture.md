...
# Binding Crate Architecture

Structure patterns for language binding crates in polyglot Rust projects.
...
packages/ directory: python/ (PyPI), typescript/ (npm), ruby/ (RubyGems), php/ (Composer), go/ (Go module), java/ (Maven), csharp/ (.NET NuGet)
...
- Duplicating core logic in binding layer
